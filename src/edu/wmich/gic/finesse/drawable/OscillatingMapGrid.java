...
public class OscillatingMapGrid {
	...
	public void update(GameContainer gc, int delta) {
		...
		for(Control c: color) c.update(time);
		
		// Update background pulses
		for(Control c: pulses) c.update(time);
	}
	
	public Color getColor(double min, double max){
		return new Color(
				color[0].getColor(min,max),
				color[1].getColor(min,max),
				color[2].getColor(min,max)
			);
	}
	...
	private class Control {
		...
		void change() {
			if(jump) start = FinesseGame.rand.nextDouble();
			else start = end;
		...
		void drawPulse(Graphics g, int pos, int i) {
			...
			g.setColor(mg.getColor(bright1 * intensity, defaultBrightness + bright2 * intensity));
